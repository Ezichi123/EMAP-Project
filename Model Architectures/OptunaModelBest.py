...
class EMAPDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        loop_onehot = np.zeros(4, dtype=np.float32)
        loop_onehot[int(sample['loop_num']) - 1] = 1.0
        loop_tensor = torch.FloatTensor(loop_onehot)   # shape (4,) instead of (1,)
...
dummy_loop = torch.randn(4, 4).to(device)
...
def masked_combined_loss(output, target, mask, lambda_diff=1.0):
    """
    MSE on values + lambda * MSE on first differences (trajectory shape).
    All terms respect the padding mask.

    output, target, mask: (batch, seq_len)
    """
    # ---- Term 1: standard masked MSE on absolute values ----
    sq_err = ((output - target) ** 2) * mask
    mse_term = sq_err.sum() / mask.sum()

    # ---- Term 2: masked MSE on first differences ----
    # diff[t] = x[t] - x[t-1], valid only where BOTH t and t-1 are real
    pred_diff = output[:, 1:] - output[:, :-1]      # (batch, seq_len-1)
    targ_diff = target[:, 1:] - target[:, :-1]      # (batch, seq_len-1)

    # a difference is real only if both adjacent timesteps are real
    diff_mask = mask[:, 1:] * mask[:, :-1]          # (batch, seq_len-1)

    diff_sq_err = ((pred_diff - targ_diff) ** 2) * diff_mask
    diff_term = diff_sq_err.sum() / diff_mask.sum()
...
def train_one_epoch(model, loader, optimizer, criterion, device):
    ...
    for topomap, phys, loop_num, treatment, target, mask in loader:
        ...
        loss = masked_combined_loss(output, target, mask, lambda_diff=1.0)
...
TRIAL_EPOCHS = 8
STUDY_TIMEOUT = 1800 # 30 minutes

# ---- Subsample train data for fast search ----
subset_frac = 0.3
n_sub = int(len(train_dataset) * subset_frac)
sub_idx = np.random.RandomState(42).choice(len(train_dataset), n_sub, replace=False)
search_train_loader = DataLoader(
    Subset(train_dataset, sub_idx),
    batch_size=BATCH_SIZE, shuffle=True, num_workers=NUM_WORKERS, pin_memory=True
)
print(f"Search uses {n_sub} of {len(train_dataset)} samples")

def build_model(trial):
    hidden_dim     = trial.suggest_categorical("hidden_dim", [64, 128, 256])
    num_gru_layers = trial.suggest_int("num_gru_layers", 1, 3)
    dropout        = trial.suggest_float("dropout", 0.2, 0.6)
    return EEGArousalModel(
...
def objective(trial):
    ...
    lambda_diff = trial.suggest_float("lambda_diff", 0.0, 2.0)
    ...
    for epoch in range(1, TRIAL_EPOCHS + 1):
        model.train()
        for topomap, phys, loop_num, treatment, target, mask in search_train_loader:
            topomap, phys = topomap.to(device), phys.to(device)
            loop_num, treatment = loop_num.to(device), treatment.to(device)
            target, mask = target.to(device), mask.to(device)
            optimizer.zero_grad()
            output = model(topomap, phys, loop_num, treatment)
            loss = masked_combined_loss(output, target, mask, lambda_diff=lambda_diff)
            loss.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
            optimizer.step()
        val_loss, val_rmse = evaluate(model, val_loader, None, device, scaler_arousal)
        scheduler.step(val_loss)
        best_rmse = min(best_rmse, val_rmse)
        print(f"  trial {trial.number} epoch {epoch}: val_rmse={val_rmse:.4f}")
...
study.optimize(objective, timeout=STUDY_TIMEOUT, show_progress_bar=True)

print("\nBest val RMSE:", study.best_value)
print("Best params:", study.best_params)
study.trials_dataframe().to_csv("models/optuna_step3_trials.csv", index=False)

# %%
bp = study.best_params   # or paste the dict manually:
# bp = {'lr': 0.0015195652634318038, 'lambda_diff': 0.8786699885770244,
#       'hidden_dim': 256, 'num_gru_layers': 1, 'dropout': 0.5899983682363993}

model = EEGArousalModel(
    cnn_feature_dim=256,
    hidden_dim=bp["hidden_dim"],
    num_gru_layers=bp["num_gru_layers"],
    dropout=bp["dropout"],
    use_treatment=True,
).to(device)

optimizer = Adam(model.parameters(), lr=bp["lr"], weight_decay=1e-4)
scheduler = ReduceLROnPlateau(optimizer, mode='min', patience=5, factor=0.5)
LAMBDA = bp["lambda_diff"]
EXPERIMENT = "step3_optuna_best"
DESCRIPTION = f"Optuna best {bp}, 50 epochs full data, first-diff loss"
BEST_MODEL_PATH = f"models/{EXPERIMENT}_best.pt"
...
# %%
import pandas as pd
labels = pd.read_csv('topomap_data/train/labels.csv')
val_labels_df = pd.read_csv('topomap_data/val/labels.csv')

train_file_sizes = labels.groupby('filename').size()
val_file_sizes = val_labels_df.groupby('filename').size()

print("TRAIN file length — max:", train_file_sizes.max(), "| 99th pct:", int(train_file_sizes.quantile(0.99)))
print("VAL file length   — max:", val_file_sizes.max(), "| 99th pct:", int(val_file_sizes.quantile(0.99)))
print("Files > 192:", (train_file_sizes > 192).sum() + (val_file_sizes > 192).sum())
